...
public class HttpRequest {
	...
	public HttpRequest withData(HashMap<String, String> params) throws IOException {
		...
		for (Map.Entry<String, String> entry : params.entrySet()) {
			result.append((result.length() > 0 ? "&" : "") + entry.getKey() + "=" + entry.getValue());//appends: key=value (for first param) OR &key=value(second and icon_more_white)
		}
